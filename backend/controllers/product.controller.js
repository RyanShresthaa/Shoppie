...
export const createProductController = async(request,response)=>{
    try {
        ...
        if(!name || !image[0] || !category[0] || !subCat?.[0] || !unit || !price || !description ){
...
export const getProductController = async(request,response)=>{
    try {
        ...
        let page = Number(src.page) || 1
        let limit = Number(src.limit) || 10
        ...
        if (published !== undefined && published !== "") {
            query.publish = published === "true" || published === true
        }
...
export const getProductByIdController = async(request,response)=>{
    try {
        const product = await ProductModel.findById(request.params.id).populate("category").populate("subcategory")
        if(!product){
